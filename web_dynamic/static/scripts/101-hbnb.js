$('document').ready(() => {
  ...
  function displayPlaces (data, stat, xml) {
    ...
    data.map(async (place) => {
      ...
      const userUrlById = usersUrl + place.user_id;
      await $.ajax(userUrlById, usersSettings);
      let user = users.find((itm) => {return itm.id === place.user_id});
      // console.log(user);
      /*if (!user) {
	console.log("entered");
        for (let i = 0; i > 0; i++) {
	  console.log('in loop');
	  user = users.find((itm) => {return itm.id === place.user_id});
	  user ? i = -2: i = 1;
	};
      }*/
